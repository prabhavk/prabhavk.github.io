...
type SeriesResp = {
  job_id: string;
  roots: string[]; // all roots available for this job
  root?: string;   // canonical/selected root (if server picks one)
...
export default function ViolinInitPage() {
  const [job, setJob] = useState<string>("");

  // Panel 1 (existing shape): pick one root; compare methods — but using ll_init
  ...
  // Panel 2 (by-root for chosen method) — ll_init
  ...
  // Cache: include metric so it doesn't collide with ll_final page
  // key = `${job}|init|${method}|${root}`
  const byRootCacheRef = useRef<Record<string, number[]>>({});

  // ---- EXPORT / DOWNLOAD STATE & HELPERS ----
  const exportPrefix = useMemo(() => buildExportPrefix?.({ job }) ?? (job || "violin_init"), [job]);
  ...
  // ===== Panel 1: Fetch data (and roots) for current job/root — metric=init =====
  const load = useCallback(async () => {
    ...
    try {
      ...
      u.searchParams.set("metric", "init");
  ...
  // Auto-load when job/root change (panel 1)
  useEffect(() => {
    if (job) void load();
  }, [job, root, load]);
  ...
  // Build violin traces for panel 1 (root fixed; compare methods) — ll_init
  ...
  const medianLineTrace: Partial<Data> | null = useMemo(() => {
    ...
    return {
      ...
      y: medians,
      line: { color: "#111827", width: 2 }, // near-black so it stands out from fills
  ...
  const layout: Partial<Layout> = {
    title: { text: `ll_init · root=${root || "(none)"}`, font: { color: "black" } },
    xaxis: {
    ...
    yaxis: {
      title: { text: "ll_init", font: { color: "black" } },
      tickfont: { color: "black" },
      zerolinecolor: "#e5e7eb",
      gridcolor: "#f3f4f6",
    },
  ...
  const layoutByRoot: Partial<Layout> = useMemo(
    () => ({
      title: { text: `ll_init · by root · method=${methodSel}`, font: { color: "black" } },
      xaxis: {
  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto bg-white text-black">
      <h1 className="text-2xl font-bold mb-4">Violin Plots of Initial Log Likelihood (ll_init)</h1>
      ...
      <div className="flex flex-wrap gap-3 items-end mb-4">
        ...
        <div className="ml-auto flex gap-2">
          <button
            ...
            onClick={() => downloadViolin("ll_init_violin", "png")}
            disabled={!data}
      ...
      {/* Panel 1: one root, compare methods (ll_init) */}
      {loading ? (
        ...
        <>
          ...
          <div className="bg-white border rounded p-2 mb-8">
            <Plot
              ...
              onInitialized={registerViolin("ll_init_violin")}
              onUpdate={registerViolin("ll_init_violin")}
            />
          </div>
        </>
      ) : (
      ...
      {/* Panel 2 — by-root distribution for selected method (ll_init) */}
      <div className="flex flex-wrap gap-3 items-center mb-3">
        ...
        <div className="ml-auto flex gap-2">
          <button
            className="px-4 py-2 rounded bg-black text-white disabled:opacity-50"
            onClick={() => downloadViolin("ll_init_by_root", "png")}
      ...
      {byRootLoading ? (
        ...
        <div className="bg-white border rounded p-2">
          <Plot
            ...
            config={{ displayModeBar: false, responsive: true }}
            style={{ width: "100%", height: "100%" }}
            onInitialized={registerViolin("ll_init_by_root")}
            onUpdate={registerViolin("ll_init_by_root")}
          />
        </div>
      ) : (
