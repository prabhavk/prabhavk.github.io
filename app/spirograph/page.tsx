// app/spirograph/page.tsx
"use client";

import React, { useCallback, useEffect, useMemo, useRef, useState } from "react";
import dynamic from "next/dynamic";
import type { PlotParams } from "react-plotly.js";
import type { Data, Layout, Config, ScatterData, PlotlyHTMLElement, PlotData } from "plotly.js";
import { buildExportPrefix } from "@/lib/exportName";

const Plot = dynamic<PlotParams>(() => import("react-plotly.js"), { ssr: false });

// Allow per-point angles for text labels
type ScatterTextAngle = { textangle?: number[] | number | "auto" };
type ScatterDataX = ScatterData & ScatterTextAngle; // extend the Scatter trace

// NOTE: keep this for UI, but don't *validate* server data against it.
...
function colorFromScale(
  ...
  for (let i = 1; i < scale.length; i++) {
    ...
    if (t <= t2) {
      ...
      const g = Math.round(g1 + u * (r2 - r1));
      const b = Math.round(b1 + u * (r2 - r1));
      return `rgb(${r},${g},${b})`;
    }
  }
...
export default function RhodoneaPage() {
  ...
  const load = useCallback(async () => {
    ...
    setErr(null);
    try {
      const u = new URL("/api/rhodonea", window.location.origin);
      ...
      if (!isApiRhodonea(json)) {
        throw new Error("Invalid /api/rhodonea response");
      }
  ...
  const byRootRepAll = useMemo(() => {
    const map = new Map<string, Map<number, SpirographRow>>();
    for (const r of rows) {
  ...
  /** Text labels placed just beyond each petal tip, oriented tangentially. */
  function buildOuterLabelsTrace(
  byRoot: Map<string, Map<number, SpirographRow>>,
  ...
  for (let i = 0; i < n; i++) {
    ...
    // outermost ring index (same as petals)
    let outerRank = -1;
  ...
  return {
    ...
    y: ys,
    text: texts,
    textposition: "middle center", // keep centered; no per-point rotation
  ...
  // ---------- Traces (Rhodonea petals + selected markers + labels) ----------

  // One label trace reused across all panels (same geometry & outermost radii)
  ...
  const initTraces = useMemo<Partial<ScatterData>[]>(() => {
    ...
    const petals = buildRhodoneaPetalsTraces(
      { title: "ll_init", colorScale: BLUES_SCALE, cmin: mn, cmax: mx },
      byRootRepAll, repRankAll, avgMap,
      { k: NODES.length, power: 2.0, widthFrac: 1.0, samples: 144, coreRadius, r0, dr }
    );
    const markers = buildMarkersTraceEquidistant(
      { title: "ll_init", values: (r) => r.ll_init, colorScale: BLUES_SCALE, cmin: mn, cmax: mx },
      byRootRepAll, repRankAll, selectedReps,
      { coreRadius, r0, dr }
    );

    return [...petals, markers, ...(labelTrace ? [labelTrace as Partial<ScatterData>] : [])];
  }, [rows, byRootRepAll, repRankAll, selectedReps, coreRadius, r0, dr, labelTrace]);
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 max-w-[1400px] mx-auto space-y-4">
      <h1 className="text-2xl font-bold">Spirographs</h1>
...
function makeNodeAvgMap(
  byRootRepAll: Map<string, Map<number, SpirographRow>>,
  selectedReps: number[],
  values: (r: SpirographRow) => number | null
): Map<string, number> {
...
function buildRhodoneaPetalsTraces(
  cfg: {
    title: string;
  ...
  const k = Math.max(1, opts?.k ?? NODES.length);
  const power = Math.max(1, opts?.power ?? 2.0);
  const widthFrac = Math.min(1, Math.max(0.2, opts?.widthFrac ?? 1.0));
  const samples = Math.max(72, Math.min(512, opts?.samples ?? 144));
  ...
  const dTheta = (2 * Math.PI) / n;
  const halfWidth = 0.5 * (Math.PI / k) * widthFrac;
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= samples; j++) {
      const th = theta0 - halfWidth + (2 * halfWidth) * (j / samples);
      const rose = Math.max(0, Math.cos(k * (th - theta0)));
      const r = coreRadius + span * Math.pow(rose, power);
    ...
    traces.push({
      ...
      line: { width: 0.6, color: "rgba(255,255,255,0.10)" },
...
/** Overlay replicate markers for the SELECTED reps. */
function buildMarkersTraceEquidistant(
  cfg: {
    title: string;
    values: (r: SpirographRow) => number | null;
    colorScale: [number, string][];
  ...
  repRankAll: Map<number, number>,
  selectedReps: number[],
  geometry: { coreRadius: number; r0: number; dr: number }
): Partial<ScatterData> {
  const xs: number[] = [];
  const ys: number[] = [];
  const colors: number[] = [];
  ...
  for (let i = 0; i < nArms; i++) {
    ...
    for (let k = 0; k < m; k++) {
      const t = (k + 1) / (m + 1);
      const r = base + t * (Rmax - base);

      const row = perRep.get(repsHere[k])!;
      const v = cfg.values(row);
      if (v == null || !Number.isFinite(v)) continue;

      xs.push(r * Math.cos(theta0));
      ys.push(r * Math.sin(theta0));
      colors.push(v);
...
/** Plot card with a small white center disk to anchor petals visually. */
function ChartCard({
  title,
  traces,
  onReady,
