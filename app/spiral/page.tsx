...
/* -------------------- Constants ---------------------- */
// 17 internal nodes: h_21..h_37  (one petal per node)
const NODES = Array.from({ length: 17 }, (_, i) => `h_${21 + i}`);

// Plotly sequential scales for nodes
const BLUES_SCALE: [number, string][] = [
  [0.0,  "#f7fbff"], [0.125,"#deebf7"], [0.25, "#c6dbef"], [0.375,"#9ecae1"],
  [0.5,  "#6baed6"], [0.625,"#4292c6"], [0.75, "#2171b5"], [0.875,"#08519c"], [1.0,  "#08306b"],
];

const REDS_SCALE: [number, string][] = [
  [0.0,  "#fff5f0"], [0.125,"#fee0d2"], [0.25, "#fcbba1"], [0.375,"#fc9272"],
  [0.5,  "#fb6a4a"], [0.625,"#ef3b2c"], [0.75, "#cb181d"], [0.875,"#a50f15"], [1.0,  "#67000d"],
];

// Geometry used for radial layout (no twist)
const R0 = 0.6;   // base radius (rank 0)
const DR = 0.35;  // radial step per rank
...
export default function SpiralPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!ct.includes("application/json")) {
        ...
        throw new Error(`Expected JSON from ${u.pathname}, got ${ct || "unknown"} (HTTP ${res.status}). ${body.slice(0, 160)}`);
      }
  ...
  const onResample = useCallback(() => { setSelectedReps(sampleReps(repsAll, 5)); }, [repsAll]);
  ...
  const coords = useMemo(() => {
    ...
    // NO twist: all ranks lie on the same ray for a given arm
    NODES.forEach((node, armIdx) => {
      const base = armIdx * dTheta;               // center angle for this arm
      const th = -base;                           // clockwise for display
      for (let k = 0; k < 5; k++) {               // ranks 0..4
        const r = R0 + k * DR;
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 max-w-[1400px] mx-auto space-y-4">
      <h1 className="text-2xl font-bold">Flower Plots (one petal per root)</h1>
...
// One semi-transparent rounded petal per root.
// Petal length uses the outermost selected rank (so all petals share the same length).
function buildSinglePetalPerRootTraces(args: {
  byRootRep: Map<string, Map<number, SpiralRow>>;
  repRanks: Map<number, number>;
  selectedReps: number[];
  options?: {
    petalWidthFrac?: number; // fraction of arm gap used by petal width
    tipCapFrac?: number;     // how much to bulge the tip
    tipPower?: number;       // roundness of tip
    coreRadius?: number;     // inner radius (white center lives inside)
    arcSamples?: number;     // angular resolution for petal outline
    stroke?: string;
    fill?: string;
  };
}): Partial<ScatterData>[] {
  const { byRootRep, repRanks, selectedReps, options } = args;

  const nArms = NODES.length;
  const dTheta = (2 * Math.PI) / nArms;

  // Outermost selected rank (same for all arms so the flower is symmetric)
  const maxRankSel = selectedReps.length ? Math.max(...selectedReps.map((r) => repRanks.get(r) ?? 0)) : 0;

  // Petal geometry options
  const petalWidthFrac = Math.min(1, Math.max(0.2, options?.petalWidthFrac ?? 0.92));
  const halfW = 0.5 * petalWidthFrac * dTheta;

  const tipCapFrac = Math.min(1, Math.max(0, options?.tipCapFrac ?? 0.60));
  const tipPower   = Math.max(0.5, options?.tipPower ?? 1.4);

  const coreRadius = Math.max(0.02, options?.coreRadius ?? 0.085);
  const arcSamples = Math.max(24, Math.min(256, options?.arcSamples ?? 64));

  const stroke = options?.stroke ?? "rgba(255,255,255,0.10)";
  const fill   = options?.fill   ?? "rgba(255,255,255,0.06)";
  ...
  for (let armIdx = 0; armIdx < nArms; armIdx++) {
    ...
    const perRep = byRootRep.get(node);
    if (!perRep) {
      // Even if this node has no data among selected reps, still draw a petal for symmetry
      // (you can `continue` here to skip instead)
    }

    const thetaCenter = armIdx * dTheta;       // center angle (radians, CCW)
    const thOut = -thetaCenter;                 // CW for display

    // Inner→outer radii: from a small core to slightly beyond the outermost ring for a nice rounded tip
    const rIn  = coreRadius;
    const rOut = R0 + (maxRankSel + 1) * DR;   // +1 to edge beyond the last ring
    const capH = tipCapFrac * DR;              // tip bulge height

    // Build polygon around the petal
    const xs: number[] = [];
    const ys: number[] = [];

    // Outer rounded tip: left → right
    for (let i = 0; i <= arcSamples; i++) {
      const a = -1 + (2 * i) / arcSamples;             // [-1, +1]
      const bump = Math.pow(1 - a * a, tipPower);      // 0 at edges, 1 at center
    ...
    // Right side back to inner base along straight-ish edge
    const sideSteps = Math.max(8, Math.floor(arcSamples / 3));
    for (let i = 1; i <= sideSteps; i++) {
      const t = i / sideSteps; // 0→1
      const th = (thOut + halfW) * (1 - t) + (thOut + halfW) * t; // constant angle (kept for clarity)
      const r  = rOut * (1 - t) + rIn * t;
      xs.push(r * Math.cos(th));
      ys.push(r * Math.sin(th));
    }

    // Inner base arc (near the core): right → left
    const innerSteps = Math.max(16, Math.floor(arcSamples / 2));
    ...
    // Left side back to the outer tip
    for (let i = 1; i <= sideSteps; i++) {
      const t = i / sideSteps;
      const th = (thOut - halfW) * (1 - t) + (thOut - halfW) * t;
      const r  = rIn * (1 - t) + rOut * t;
      xs.push(r * Math.cos(th));
      ys.push(r * Math.sin(th));
    }
...
// Nodes: keep colored by the metric, show colorbar
...
function ChartCard({
  ...
  traces,
  onReady,
  centerRadius = 0.09, // small white center
}: {
  title: string;
  traces: Partial<Data>[];
