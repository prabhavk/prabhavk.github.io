...
/* Make header-safe: inline to avoid multiple definitions across TUs */
inline void flatten_json(const json& j,
...
namespace emtr {
    ...
    // Format a double for JSON (NaN/Inf => "null")
    ...
    inline void emit_row_json(
        ...
        const std::string s_ll_initial   = json_number(ll_initial);
        const std::string s_ecd_ll_first = json_number(ecd_ll_first);
        const std::string s_ecd_ll_final = json_number(ecd_ll_final);
        const std::string s_ll_final     = json_number(ll_final);

        // fflush for prompt delivery to worker
        std::printf(
            "%s"
            "{\"method\":\"%s\",\"root\":\"%s\",\"repetition\":%d,"
            "\"iter\":%d,\"ll_initial\":%s,"
            "\"ecd_ll_first\":%s,\"ecd_ll_final\":%s,\"ll_final\":%s}\n",
            tag,
            im.c_str(), rt.c_str(), repetition, iter,
            s_ll_initial.c_str(),
            s_ecd_ll_first.c_str(),
            s_ecd_ll_final.c_str(),
            s_ll_final.c_str());
        std::fflush(stdout);
    }
    ...
    struct prim_graph {
        ...
        : n(n_), W(static_cast<std::size_t>(n_) * static_cast<std::size_t>(n_),
            ...
            for (int k = 0; k < m; ++k) {
                ...
                double w = weights[k];
                if (!(0 <= u && u < n && 0 <= v && v < n))
                    throw std::runtime_error("prim_graph: invalid edge endpoints"); // <-- fixed
